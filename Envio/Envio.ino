//***********************************//
//Modulo medicion nivel continuo     //
//                                   //
//Matias Pierdominici                //
...
const char MqttUser[]="medicionNivelContinuo";
const char MqttPassword[]="1234";
const char MqttClientID[]="medicionNivelContinuo";
...
void callback(char* topic, byte* payload, unsigned int length)
{
  ...
  if(!strcmp(topic,"nivelContinuo/requestData")){
    Serial.print('a');
  }
    ...
    if (mqtt_client.connect(MqttClientID,MqttUser,MqttPassword))
      {
            ...
            mqtt_client.subscribe("nivelContinuo/requestData");
...
void setup() {
  ...
  setUpWifi(ssid,pass);
  setUpMqtt();
  pinMode(PIN_NIVEL_MAXMIN,INPUT);
...
void loop() {
 ...
 if(Serial.available()){
  nivelActual=Serial.read();
  mqtt_client.publish("nivelContinuo/nivel/continuo",(String(nivelActual)).c_str());
  if(digitalRead(PIN_NIVEL_MAXMIN)){
     nivelMaxMin=1;
  }else{
     nivelMaxMin=0;
  }
  mqtt_client.publish("nivelContinuo/nivel/maxmin",(String(nivelMaxMin)).c_str());
